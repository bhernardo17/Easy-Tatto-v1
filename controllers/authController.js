...
app.post('/login', async (req, res) => {
    ...
    try {
        const user = await User.findOne({ 
            $or: [{ email: username }, { nome: username }] // Busca por email ou username
        });
        if (user && (await bcrypt.compare(password, user.senha))) { // Verificação de senha
            console.log(`✅ Login bem-sucedido: ${user.email || user.nome}`);
            res.redirect('/feed'); // Redireciona para o feed
        } else {
...
app.post('/cadastro', async (req, res) => {
    ...
    try {
        // Verifica se o e-mail ou username já estão registrados
        const existingUser = await User.findOne({ 
            $or: [{ email }, { nome: username }] 
        });
        if (existingUser) {
            console.error('Erro: Usuário ou E-mail já registrado');
        ...
        await newUser.save();
        console.log('✅ Usuário cadastrado:', newUser.email || newUser.nome);
